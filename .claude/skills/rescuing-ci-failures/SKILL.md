...
This is the lean adaptation of the `picosystem/rdp` skill: same algorithm and
stop conditions, but tuned to **this** project's three jobs. **No helper script
...
- The new `build-macos` / `build-windows` jobs red on their first run (see
  Known first-run failures — they were added blind and not yet CI-verified).
...
| `Configure` | `compiler-error` (CMake configure error, missing dep, generator) |
| `Build` | `compiler-error` (warnings-as-errors, link errors) |
| `Test` | `test-failure` (incl. Windows DLL-load failures — see below) |
...
| `SDL3.dll` not found / `0xc0000135` / "code execution cannot proceed" (Windows `Test` step, exe can't load its DLL) | `test-failure` (runtime-DLL — root cause is packaging; see Known first-run failures) |
...
`build-macos` and `build-windows` were added without ever being built (no
Apple/Windows host available). These are the high-probability first-run failures
and their standard fixes — give them to the fix subagent as priors:

| Symptom | Job | Standard fix |
|---|---|---|
| `LNK2038` "mismatch detected for 'RuntimeLibrary'" (gtest `/MT` vs project `/MD`) | build-windows | `set(gtest_force_shared_crt ON CACHE BOOL "" FORCE)` in `cmake/Dependencies.cmake`, **inside the existing `if(BUILD_TESTS)` block**, just before its `FetchContent_MakeAvailable(googletest)` |
| Test exe fails to start: `SDL3.dll` missing / `0xc0000135` | build-windows | Copy runtime DLLs next to test/exe via a POST_BUILD `$<TARGET_RUNTIME_DLLS:…>` copy, or set `SDL_SHARED OFF` / `SDL_STATIC ON` for SDL3 |
| `error C4267` / `C4244` / `C4100` (MSVC `/W4 /WX`) | build-windows | Minimal correct fix in our `src/*.cc` (explicit cast / `(void)param` / type fix) — never widen scope, never drop `/WX` |
| `error: …` from `-Werror` (AppleClang stricter than Linux clang) | build-macos | Minimal correct fix in the flagged `src/*.cc` |

These edits touch `cmake/Dependencies.cmake` or `src/*.cc`, never the warning
flags themselves (`/W4 /WX`, `-Werror` via `COMPILE_WARNING_AS_ERROR`) — the
point of the jobs is to keep warnings fatal.
...
- **Use a dedicated `build-rescue/` dir** (the command above), not the user's
  `build/`. The default `build/` is `Debug` + plain `clang++` (Makefile defaults);
  pinning `clang-22`/`Release` in it would force a reconfigure and clobber the
  user's working build.
- **Pin clang-22.** CI uses `clang-22`/`clang++-22`; the Makefile default
  `clang++-22` matches CXX but CI also pins the C compiler. Pin both for fidelity.
...
- Do NOT weaken the warning posture: never drop /W4, /WX, -Wall/-Wextra/-Wpedantic,
  or COMPILE_WARNING_AS_ERROR to make a warning go away — fix the code.
...
- `cmake/Dependencies.cmake` — FetchContent (SDL3 / SDL_mixer / ImGui / mINI /
  gtest); home of the gtest-CRT and SDL-shared fixes.
