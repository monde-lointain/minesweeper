...
static void app_new_game(struct AppState *s) {
    ...
    game_reset(&s->board, w, h, mines, NULL, NULL);
...
SDL_AppResult app_init(struct AppState **out, int argc, char **argv) {
    ...
    if (s->settings.window_x >= 0 && s->settings.window_y >= 0) {
        SDL_SetWindowPosition(s->window, s->settings.window_x, s->settings.window_y);
    }
...
SDL_AppResult app_iterate(struct AppState *s) {
    ...
    ui_dialogs(&s->settings, &actions, &s->show_custom, &s->show_best, &s->show_about,
               &s->show_name, s->pending_name_level);
    ...
    render_frame(s->renderer, &s->assets, &s->board, &s->settings, &lay, s->button_face, s->press_x,
                 s->press_y, s->elapsed_sec);
    ImGui::Render();
