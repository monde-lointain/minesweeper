...
/* Win95 chrome colors. */
enum {
    FACE_R = 192, FACE_G = 192, FACE_B = 192,
...
static void draw_bevel(SDL_Renderer *r, int x, int y, int w, int h, int width, int scale,
                       bool raised) {
    int i;
    int tl = raised ? HI : SH;
    int br = raised ? SH : HI;
...
static void blit(SDL_Renderer *r, SDL_Texture *tex, SDL_FRect src, int dx, int dy, int dw, int dh) {
    SDL_FRect dst;
    dst.x = (float)dx;
    dst.y = (float)dy;
    dst.w = (float)dw;
...
static void draw_led3(SDL_Renderer *r, SDL_Texture *tex, int x, int y, int scale, int value) {
...
static int cell_sprite(const struct Board *b, int x, int y, int press_x, int press_y) {
...
void render_frame(SDL_Renderer *renderer, const struct Assets *a, const struct Board *b,
    ...
    draw_bevel(renderer, 0, top, lay->window_w, lay->window_h - top, 3, scale, true);

    /* Recessed 3px bevel around the grid. */
    draw_bevel(renderer, lay->grid_x - 3 * scale, lay->grid_y - 3 * scale, gw + 6 * scale,
               gh + 6 * scale, 3, scale, false);
    ...
    draw_bevel(renderer, lay->bomb_led_x - scale, lay->led_y - scale, 3 * led_w + 2 * scale,
               led_h + 2 * scale, 1, scale, false);

    /* Recessed 1px bevel around the time LED panel. */
    draw_bevel(renderer, lay->time_led_x - scale, lay->led_y - scale, 3 * led_w + 2 * scale,
               led_h + 2 * scale, 1, scale, false);

    /* Raised 1px bevel around the smiley button. */
    draw_bevel(renderer, lay->button_x - scale, lay->button_y - scale, btn + 2 * scale,
               btn + 2 * scale, 1, scale, true);

    /* 2. Bomb-count LED (left). ------------------------------------------- */
    draw_led3(renderer, a->led, lay->bomb_led_x, lay->led_y, scale, game_mines_remaining(b));
    ...
    /* 4. Smiley button. ---------------------------------------------------- */
    {
        ...
        blit(renderer, a->button, assets_button_rect(face), lay->button_x, lay->button_y, btn, btn);
    }
