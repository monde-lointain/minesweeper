...
float ui_menu_bar(const struct Settings *s, struct UiActions *out) {
    ...
    if (ImGui::BeginMainMenuBar()) {
        if (ImGui::BeginMenu("Game")) {
            ...
            if (ImGui::MenuItem("Beginner", NULL,
            ...
            if (ImGui::MenuItem("Expert", NULL,
                                s->difficulty == DIFF_EXPERT)) {
...
void ui_dialogs(const struct Settings *s, struct UiActions *out,
    ...
    if (ImGui::BeginPopupModal("Best Times", NULL,
        ...
        for (int i = 0; i < LEVEL_COUNT; ++i) {
            ImGui::Text("%-14s %3d seconds   %s", level_names[i],
                        s->best_time[i], s->best_name[i]);
        }
        ImGui::Separator();
