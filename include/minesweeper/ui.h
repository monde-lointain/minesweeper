 ...
 *
 * ImGui's C++ API is confined to ui.cc/app.cc with per-line HERESY suppressions.
 * ui fills a POD UiActions each frame; app applies the requested actions. Dialog
 * layouts mirror winmine pref.dlg (Custom field, Best Times, Enter Name, About).
 */
 ...
 * when the dialog closes and writes results into `out`. `level_for_name` selects
 * which difficulty's record the Enter-Name modal is recording. */
