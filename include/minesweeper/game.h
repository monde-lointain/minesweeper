...
/* Both-button chord on a revealed number: if adjacent FLAG_MINE count == number,
 * reveal all non-flagged neighbours. Returns enum RevealResult. */
int game_chord(struct Board *b, int x, int y);

/* Right-click cycle. With marks: none->mine->question->none; without: none->mine->none. */
